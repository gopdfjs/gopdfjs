...
### 8. AI Chat with PDF
*   **Status**: Pending — see `pending/0047-chat-with-pdf.md`.
*   **Strategy (browser-only)**: Extract text via `pdf.js`, embed and store chunks **in-browser** (transformers.js + client vector store), and run generation on an **in-browser LLM** (WebLLM / transformers.js, WASM/WebGPU). No cloud LLM API — document content never leaves the browser. Deferred pending in-browser model maturity.
