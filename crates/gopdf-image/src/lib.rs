...
mod tests {
    ...
    #[test]
    fn encode_images_rejects_mismatched_dimensions() {
        let err = encode_images(&[0u8; 16], &[2], &[2, 2], "jpeg", 80).unwrap_err();
        assert!(err.contains("same length"));
    }

    #[test]
    fn encode_images_rejects_unknown_format() {
        let pixels = vec![255u8; 16];
        let err = encode_images(&pixels, &[2], &[2], "webp", 80).unwrap_err();
        assert!(err.contains("unsupported format"));
    }

    #[test]
    fn encode_images_jpeg_length_prefix_and_roundtrip() {
        // 2×2 不透明红块
        let pixels: Vec<u8> = vec![
            255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255,
        ];
        let packed = encode_images(&pixels, &[2], &[2], "jpeg", 85).unwrap();
        assert!(packed.len() > 4);
    ...
    #[test]
    fn encode_images_png_length_prefix() {
        let pixels: Vec<u8> = vec![
            0, 128, 255, 255, 0, 128, 255, 255, 0, 128, 255, 255, 0, 128, 255, 255,
        ];
        let packed = encode_images(&pixels, &[2], &[2], "png", 0).unwrap();
        let len = u32::from_be_bytes([packed[0], packed[1], packed[2], packed[3]]) as usize;
        assert_eq!(len + 4, packed.len());
        image::load_from_memory_with_format(&packed[4..], ImageFormat::Png).expect("decode png");
    }

    #[test]
    fn grayscale_pdf_without_images_passes_through() {
        let pdf = b"%PDF-1.4\n% no embedded raster images\n%%EOF\n";
        let out = grayscale_pdf(pdf).unwrap();
        assert_eq!(out.as_slice(), pdf.as_slice());
    }

    #[test]
    fn grayscale_pdf_converts_embedded_jpeg_to_luma() {
        let jpeg = tiny_color_jpeg();
        ...
        let eoi = out[soi..]
            ...
            .expect("jpeg eoi");
        let gray_jpeg = &out[soi..soi + eoi + 2];
        let decoded = image::load_from_memory_with_format(gray_jpeg, ImageFormat::Jpeg)
    ...
    fn find_bytes_locates_needle() {
        assert_eq!(find_bytes(b"abc stream xyz", b"stream"), Some(4));
