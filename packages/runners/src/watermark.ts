...
export async function watermarkPdf(
  file: File,
  content: { text?: string; image?: File },
  ...
  if (content.image) {
    const imgBytes = await readFileAsArrayBuffer(content.image);
    embeddedImage = content.image.type === "image/png" ? await doc.embedPng(imgBytes) : await doc.embedJpg(imgBytes);
  }
