...
describe("splitPdf", () => {
  it("should split a document containing a real PDF page and mock pages", async () => {
    ...
    const pdf = new File([docBytes.buffer], "combined.pdf", { type: "application/pdf" });
    
    // Split: Part 1 = [Page 1 (Real)], Part 2 = [Pages 2-5 (Mock)]
    const ranges = [[0], [1, 2, 3, 4]];
    
    const chunks = await splitPdf(pdf, ranges);
