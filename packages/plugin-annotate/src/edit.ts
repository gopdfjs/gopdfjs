...
export async function applyEdits(bytes: Uint8Array, annotations: Annotation[]): Promise<Uint8Array> {
  ...
  for (const ann of annotations) {
    ...
    if (ann.type === "image" && ann.image) {
      const embedded =
        ann.image.mimeType === "image/png"
          ? await doc.embedPng(ann.image.bytes)
          : await doc.embedJpg(ann.image.bytes);
