...
async function resolveOutline(
  pdf: { getOutline: () => Promise<PdfOutlineItem[] | null> },
...
export async function pdfToEpub(
  ...
  let outline = await resolveOutline(pdf as { getOutline: () => Promise<PdfOutlineItem[] | null> });
  if (outline.length === 0) {
  ...
  const epubBytes = await zip.generateAsync({ type: "uint8array", mimeType: EPUB_MIMETYPE });
  return new Blob([epubBytes], { type: EPUB_MIMETYPE });
}
