import type { GopdfRuntime, PdfTextContentItem } from "@gopdfjs/runtime";
...
export function groupTextItemsIntoRuns(items: PdfTextContentItem[], pageIndex: number): PdfTextRun[] {
  const lines: Array<{
    y: number;
  ...
  for (const item of items) {
    ...
    const width = item.width ?? Math.max(text.length * fontSize * 0.5, 8);
    const run = {
      text,
      x,
      width,
