...
description: "This guide details how the `.wsx` file type system works in GoPDF.js, maintenance methods, and best practices."
---

This guide details how the `.wsx` file type system works in GoPDF.js, maintenance methods, and best practices.
...
In GoPDF.js's monorepo structure, multiple packages (`wsx-router`, `base-components`, `examples`, etc.) all contain the same `wsx.d.ts` file, which looks redundant. But this is **required**, not a design flaw.
...
GoPDF.js chose engineering pragmatism: acknowledge technical limitations, provide best developer experience through automation tools.
