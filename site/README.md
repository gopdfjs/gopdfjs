...
2. **Automatic deploy:** Push changes under `site/` to `main` → workflow runs → site live at `https://<user>.github.io/gopdfjs/`.
...
Workflow only triggers on `site/**` changes (plus the workflow file itself). No Rust or WASM build step.
