...
name: gopdf-e2e
description: >-
  Browser E2E verification for GoPDF npm packages via Playwright on apps/demo.
...
RFC browser acceptance tests live under **`apps/demo/e2e/`** — not `.spec/`.
...
```bash
pnpm build:wasm    # single @gopdfjs/wasm/pkg (shared by both adapters; after Rust changes)
pnpm test:e2e
```
...
2. Ensure demo route exists in `apps/demo/src/`.
...
4. Copy `apps/demo/e2e/tools/compress.spec.ts` → `tools/<slug>.spec.ts`.
5. Update RFC § Tests row: `apps/demo/e2e/tools/<slug>.spec.ts`.
6. Run `pnpm test:e2e`.
