...
- **架构**：`.spec/rfc/completed/0057-rust-wasm-engine-architecture.md`
- **WASM 边界与 crate 分层**：`.spec/rfc/completed/0058-engine-plugin-charter.md`
- **Rust workspace**：`crates/README.md` — 算法在 `crates/gopdf-*`；**wasm-pack 在各 adapter**（`pnpm build:wasm` 跑 browser `web` + node `nodejs`）；根目录 `pnpm test:rust`；禁止在 `packages/engine/` 放 Rust 或 wasm 产物
